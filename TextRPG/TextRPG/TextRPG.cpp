...
int main()
{
	...
	const int iLevelUpExp[LEVEL_MAX] = { 4000, 10000, 20000, 35000, 50000, 70000, 100000
...

					{
						...
						if (tMonster.iHP <= 0)
						{
							...
							if (tPlayer.iExp >= iLevelUpExp[tPlayer.iLevel - 1])
							{
								...
								int iArmor = rand() % (tLvUpTable[iJobIndex].iArmorMax - tLvUpTable[iJobIndex].iArmorMin + 1) +
						...
						iAttack = rand() % ((tMonster.iArmorMax - tMonster.iAttackMin) + 1) +
							tMonster.iAttackMin;
