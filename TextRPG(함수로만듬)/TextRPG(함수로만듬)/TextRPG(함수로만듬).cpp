...
//입력 오류일 경우 혹은 잘못된 번호를 입려한 경우 MM_NONE반환
int OutputMainMenu()
{
	system("cls");
	cout << "******************로비******************" << endl;
...
void OutputPlayer(_tagPlayer* pPlayer)
{
	...
	//방어구를 장착하고 있을 경우 방어력에 방어구 방어력을 추가하여 추력한다.
...
void Battle(_tagPlayer* pPlayer, _tagMonster* pMonster)
{
	...
	if (pMonster->iHP <= 0)
	{
		...
		if (pPlayer->iExp >= g_iLevelUpExp[pPlayer->iLevel - 1])
		{
			...
			int iAttackUp = rand() % (g_tLvUpTable[iJobIndex].iAttackMax - g_tLvUpTable[iJobIndex].iAttackMin + 1) +
				g_tLvUpTable[iJobIndex].iAttackMin;
			int iArmor = rand() % (g_tLvUpTable[iJobIndex].iArmorMax - g_tLvUpTable[iJobIndex].iArmorMin + 1) +
				g_tLvUpTable[iJobIndex].iArmorMin;
	...
	iAttack = rand() % ((pMonster->iArmorMax - pMonster->iAttackMin) + 1) +
		pMonster->iAttackMin;
...
//맵에 관련된 동작을 처리한다.
void RumMap(_tagPlayer* pPlayer, _tagMonster* pMonsterArr)
{
	bool bLoop = true;
	while (bLoop)
	{
...
bool LoadPlayer(_tagPlayer *pPlayer)
{
	...
	if (pFile)
	{
		...
		//만야 저장할때 무기를 차고 있었다면 해당 무기정보도
		//같이 저장이 되었다. 그러므로 여기서 차고 있을 경우
		//읽어야한다
		if (pPlayer->bEquip[EQ_WEAPON])
			fread(&pPlayer->bEquip[EQ_WEAPON], sizeof(_tagItem), 1, pFile);
...
bool SavePlayer(_tagPlayer* pPlayer)
{
	...
	if (pFile)
	{
		...
		//읽어야한다
		if (pPlayer->bEquip[EQ_WEAPON])
			fwrite(&pPlayer->bEquip[EQ_WEAPON], sizeof(_tagItem), 1, pFile);
...
int main()
{
	...
	tStoreArmor[0] = CreateItem("천갑옷", IT_ARMOR, 2, 5, 1000, 500,
		"천으로 만든 허접한 갑옷");
