...
링크드리스크는 데이터 목록을 연결시켜서 접근할 수 있는 구조를 제공한다.
링크드리스크는 데이터를 저장하기 위한 노드라는 것이 존재한다.각 노드의 특징은
다음 노드를 알 수 있게 된다.(다음노드의 메모리 주소를 저장한다는 의미이다.)
리스트는 선형구조로 되어있다.그렇기 때문에 배열처럼 특정 요소에 바로 접근이 불가능하다.
무조건 앞에서부터 차례대로 타고 들어가야 한다.
노드를 새로 추가할때는 노드를 생성하고 마지막 노드에 연결만 해주면 되기 때문에 개수의
제한이 없다.
*/
...
void Insert(PLIST pList)
{
	...
	//현재 추가하는 노드는 가장 마지막에 추가될 것이기 때문에 노드가 존재하지 않는다.
	//그래서 다음 노드는 NULL로 초기화하고 저보는 위에서 입력받은 학생정보를
	//주도록한다.
	pNode->pNext = NULL;
...
//const포인터이기 때문에 가리키는 대상의 값을 변경할 수 없다.
...
void Delete(PLIST pList)
{
	...
	while (pNode != NULL)
	{
		if (strcmp(pNode->tStudent.strName, strName) == 0)
		{
			...
			//이전 노드가 있을 경우에는 이전 노드의 다음을 지운 노드의
			//다음노드를 연결해준다.
			else
			{
				delete pNode;
